# 02. Major Question Types

## Purpose
This is the core of the CheatBook - template-driven frameworks for every major question type you'll encounter in the ADIT UK exam. Each template includes structure, key considerations, and worked examples from past papers.

## Question Type Organization

### Part A Questions (25 marks each - 45 minutes per question)
**Characteristics**:
- Complex, multi-layered scenarios
- Require comprehensive analysis
- Often combine multiple tax areas
- Demand structured, professional presentation

**Topics**:
- Residence (Corporate and Individual)
- Permanent Establishment
- Remittance Basis

### Part B Questions (20 marks - 36 minutes per question)
**Characteristics**:
- Focus on anti-avoidance provisions
- International tax implications
- Require knowledge of specific legislative provisions
- Often involve calculations

**Topics**:
- Controlled Foreign Companies (CFCs)
- Thin Capitalization
- Transfer Pricing
- Business Investment Relief

### Part C Questions (15 marks each - 27 minutes per question)
**Characteristics**:
- More focused, specific topics
- Shorter, targeted answers required
- May include quick calculations
- Often test application of recent legislation

**Topics**:
- Domicile and deemed domicile
- Trusts taxation
- ATED (Annual Tax on Enveloped Dwellings)
- Withholding Tax
- Other miscellaneous topics

## Current Content

### Part A Templates
- **[Part_A_Complex_Analysis_Questions.md](./Part_A_Complex_Analysis_Questions.md)** - Comprehensive guide covering all Part A question types with worked examples from 2020-2025 exams
- **[Part_A_Residence_Template.md](./Part_A_Residence_Template.md)** - Quick reference template for Corporate and Individual Residence questions

### Part B Templates
- **[Part_B_Anti_Avoidance_Questions.md](./Part_B_Anti_Avoidance_Questions.md)** - Comprehensive guide covering all Part B anti-avoidance topics including:
  - Controlled Foreign Companies (CFCs) - 50% exam frequency
  - Transfer of Assets Abroad (ToAA/TAAR)
  - Offshore Insurance Bonds & Personal Portfolio Bonds
  - Thin Capitalisation & Transfer Pricing
  - Emerging Anti-Avoidance Regimes (DPT, Unallowable Purpose Rule)

### Part C Templates
- **[Part_C_Domicile_Template.md](./Part_C_Domicile_Template.md)** - Domicile and deemed domicile question template

## Template Structure

Each template contains:

1. **Question Recognition Triggers** - How to identify this question type
2. **Time Allocation** - Minutes per section based on 1.8 min/mark rule
3. **Answer Structure Template** - Step-by-step framework with sample wording
4. **Key Legislation References** - Sections to cite
5. **Common Variations** - Different ways the topic may appear
6. **Common Pitfalls to Avoid** - Mistakes that lose marks
7. **Marking Scheme Breakdown** - How examiners allocate marks
8. **Quick Checklist** - Final review before moving on

## How to Use This Section

### Phase 1: Template Learning (Week 3-4)
1. Read TEMPLATE.md for one topic
2. Study one worked example
3. Attempt a second example independently
4. Compare your answer to the solution

### Phase 2: Template Mastery (Week 5-6)
1. Practice timed answers (strict time limits)
2. Work through all examples in each topic
3. Identify your weak areas
4. Create personal shortcuts and mnemonics

### Phase 3: Integration (Week 7-8)
1. Mix questions from different topics
2. Practice identifying question types quickly
3. Simulate exam conditions with full papers

## Time Allocation Strategy

### Part A (25 marks = 45 minutes)
- Read and analyze: 5 minutes
- Plan structure: 3 minutes
- Write answer: 34 minutes
- Review: 3 minutes

### Part B (20 marks = 36 minutes)
- Read and analyze: 4 minutes
- Plan structure: 2 minutes
- Write answer: 27 minutes
- Review: 3 minutes

### Part C (15 marks = 27 minutes)
- Read and analyze: 3 minutes
- Plan structure: 2 minutes
- Write answer: 20 minutes
- Review: 2 minutes

## Success Metrics
- [ ] Complete all templates (13 topic areas)
- [ ] Work through minimum 3 examples per topic
- [ ] Achieve target time per question type
- [ ] Score 60%+ on practice questions

---
*Templates are frameworks, not crutches - adapt them to each unique scenario*
